...
########     Runs an SQL query and returns the result in panda dataframe     ############
def _read_sql(db_path: str, query: str, params: tuple[Any, ...] = ()) -> pd.DataFrame:
    with sq.connect(db_path) as conn:
        return pd.read_sql_query(query, conn, params=params)


#########           Same as above but designed specifically for task 8       ############
def _read_sql_with_regex(db_path:str, query: str, params: tuple[Any, ...] = ()) -> pd.DataFrame:
    conn = sq.connect(db_path)
    conn.create_function( "REGEXP",2,lambda pattern, value: 1 if value is not None and re.search(pattern, value) else 0,)
    try:
        return pd.read_sql_query(query, conn, params=params)
    finally:
        conn.close()

#######             Used when only one value needs to be returned           #############
...
###         Obtains valid format for generated database             ################### 
...
###############################################################################################################
###################             Question Coding Starts here            ########################################
###############################################################################################################

"""
The only changes that you have to make it where it is written TODO
Rest of that could be left as it is
"""


#_____________________________              Question - 01                         ____________________________________________
