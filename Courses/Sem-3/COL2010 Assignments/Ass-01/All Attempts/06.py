...
def verify_candidate_presence(db_path: str, sku_id: str, node_id: int) -> bool:
    ...
    df=_read_sql(db_path,query, (sku_id,node_id))
    result = len(df)>0
...
def calculate_shipment_manifest_mass(db_path: str, shipment_id: int) -> float:
    ...
    query = """
        SELECT SUM(shipment_manifests.manifest_quantity * inventory_items.unit_weight_kg)
        FROM shipment_manifests
        INNER JOIN inventory_items
            ON shipment_manifests.item_id = inventory_items.item_id
        WHERE shipment_manifests.shipment_id=?
    """
    # TODO: execute scalar SQL query and print the target line.
    df=_read_sql(db_path,query,(shipment_id,))

    mass=df.iloc[0,0]
    print(f"COMPUTED_MANIFEST_MASS_KG: {mass:.2f}")
    return float(mass)
...
def filter_sku_by_regex_pattern(db_path: str, exact_regex: str) -> list[str]:
    ...
    result=df["sku"].tolist()

    print(f"REGEX_MATCHED_ELEMENTS_COUNT: {len(result)}")
    return result
